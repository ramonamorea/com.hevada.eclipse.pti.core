 ...
 */
public class CheckedListDialogField extends ListDialogField<Object> {

	private int fCheckAllButtonIndex;
	private int fUncheckAllButtonIndex;

	private List<Object> fCheckElements;

	public CheckedListDialogField(IListAdapter<Object> adapter, String[] customButtonLabels, ILabelProvider lprovider) {
		super(adapter, customButtonLabels, lprovider);
		this.fCheckElements = new ArrayList<Object>();
	 ...
	 * constructor. The behaviour of the button marked as the check button will then
	...
	/**
	 * Sets the index of the 'uncheck' button in the button label array passed in
	 * the constructor. The behaviour of the button marked as the uncheck button
	 * will then be handled internally. (enable state, button invocation behaviour)
	 */
	...
	/*
	 * @see DialogField#dialogFieldChanged Hooks in to get element changes to update
	 * check model.
	 */
	...
	public void setCheckedElements(Collection<?> list) {
		fCheckElements = new ArrayList<Object>(list);
		if (isOkToUse(fTableControl)) {
